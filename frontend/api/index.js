...
const Certificate = makeModel('Certificate', { name: String, issuer: String, issueDate: String, expiryDate: String, credentialId: String, credentialUrl: String, imageUrl: String });
...
const crud = (prefix, Model) => {
  app.get([`/api/${prefix}`, `/${prefix}`], protect, async (req, res) => {
    try {
      ...
      res.json({ success: true, items: await Model.find({ portfolioId: p._id }).sort({ createdAt: -1 }) });
    } catch (err) { res.status(500).json({ success: false, message: err.message }); }
  });
