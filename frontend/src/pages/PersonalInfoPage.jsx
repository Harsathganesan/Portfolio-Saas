...
const PersonalInfoPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast('Personal details & social links saved!', 'success');
    } catch (err) {
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <User className="w-6 h-6 text-indigo-400" /> Personal Details & Socials
          </h1>
          <p className="text-xs text-slate-400">Update your header details, profile photo, and social links</p>
        </div>

        <button
          onClick={() => setAiModalOpen(true)}
          className="px-4 py-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 text-purple-300 text-xs font-semibold flex items-center gap-2 transition"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-8 text-xs">
        ...
        <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-6">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider text-indigo-400">Profile Photo & Basic Info</h2>

          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="relative group">
              {form.avatar ? (
                <img src={form.avatar} alt="Avatar" className="w-24 h-24 rounded-2xl object-cover ring-2 ring-indigo-500/40" />
              ) : (
                <div className="w-24 h-24 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="block text-slate-400 font-medium">About Me / Bio</label>
              <button
                type="button"
                onClick={() => setAiModalOpen(true)}
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 outline-none focus:border-indigo-500 leading-relaxed"
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Public Email</label>
              <input
                type="email"
                name="email"
                value={form.email}
        ...
        <button
          type="submit"
          disabled={saving}
          className="gradient-btn px-6 py-3 rounded-2xl font-semibold text-xs flex items-center gap-2 shadow-xl shadow-indigo-500/20"
        >
